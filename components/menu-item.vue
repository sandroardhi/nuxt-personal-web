<template>
  <div>
    <!-- mobile nav -->
    <nav class="relative md:hidden right-0 pr-2">
      <transition name="menu" mode="out-in">
        <div class="w-10" v-if="!menuState" @click="toggleMenu">
          <svg
            viewBox="0 -0.5 25 25"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
            <g
              id="SVGRepo_tracerCarrier"
              stroke-linecap="round"
              stroke-linejoin="round"
            ></g>
            <g id="SVGRepo_iconCarrier">
              <path
                d="M5.5 7.75C5.08579 7.75 4.75 8.08579 4.75 8.5C4.75 8.91421 5.08579 9.25 5.5 9.25V7.75ZM19.5 9.25C19.9142 9.25 20.25 8.91421 20.25 8.5C20.25 8.08579 19.9142 7.75 19.5 7.75V9.25ZM5.5 11.75C5.08579 11.75 4.75 12.0858 4.75 12.5C4.75 12.9142 5.08579 13.25 5.5 13.25V11.75ZM17.5 13.25C17.9142 13.25 18.25 12.9142 18.25 12.5C18.25 12.0858 17.9142 11.75 17.5 11.75V13.25ZM5.5 15.75C5.08579 15.75 4.75 16.0858 4.75 16.5C4.75 16.9142 5.08579 17.25 5.5 17.25V15.75ZM12.5 17.25C12.9142 17.25 13.25 16.9142 13.25 16.5C13.25 16.0858 12.9142 15.75 12.5 15.75V17.25ZM5.5 9.25H19.5V7.75H5.5V9.25ZM5.5 13.25H17.5V11.75H5.5V13.25ZM5.5 17.25H12.5V15.75H5.5V17.25Z"
                fill="#000"
                class="dark:fill-gray-200"
              ></path>
            </g>
          </svg>
        </div>
        <div class="relative" v-else @click="toggleMenu">
          <div class="w-10">
            <svg
              viewBox="0 -0.5 25 25"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
              <g
                id="SVGRepo_tracerCarrier"
                stroke-linecap="round"
                stroke-linejoin="round"
              ></g>
              <g id="SVGRepo_iconCarrier">
                <path
                  d="M6.5 11.75C6.08579 11.75 5.75 12.0858 5.75 12.5C5.75 12.9142 6.08579 13.25 6.5 13.25V11.75ZM18.5 13.25C18.9142 13.25 19.25 12.9142 19.25 12.5C19.25 12.0858 18.9142 11.75 18.5 11.75V13.25ZM6.5 15.75C6.08579 15.75 5.75 16.0858 5.75 16.5C5.75 16.9142 6.08579 17.25 6.5 17.25V15.75ZM18.5 17.25C18.9142 17.25 19.25 16.9142 19.25 16.5C19.25 16.0858 18.9142 15.75 18.5 15.75V17.25ZM6.5 7.75C6.08579 7.75 5.75 8.08579 5.75 8.5C5.75 8.91421 6.08579 9.25 6.5 9.25V7.75ZM18.5 9.25C18.9142 9.25 19.25 8.91421 19.25 8.5C19.25 8.08579 18.9142 7.75 18.5 7.75V9.25ZM6.5 13.25H18.5V11.75H6.5V13.25ZM6.5 17.25H18.5V15.75H6.5V17.25ZM6.5 9.25H18.5V7.75H6.5V9.25Z"
                  fill="#000"
                  class="dark:fill-gray-200"
                ></path>
              </g>
            </svg>
          </div>
          <div class="w-32 bg-gray-300 dark:bg-gray-700 absolute right-0 z-20" @click.stop>
            <ul class="ul-menu-mobile flex flex-col justify-center space-y-2">
              <li>
                <NuxtLink to="/" class="link-mobile">Main</NuxtLink>
              </li>
              <li>
                <NuxtLink to="/about" class="link-mobile">About</NuxtLink>
              </li>
              <li>
                <NuxtLink to="/projects" class="link-mobile">Projects</NuxtLink>
              </li>
              <li>
                <NuxtLink to="/blog" class="link-mobile">Blogs</NuxtLink>
              </li>
              <li>
                <ColorMode class="link-mobile"/>
              </li>
            </ul>
          </div>
        </div>
      </transition>
    </nav>
    <!-- end of mobile nav -->
    <nav class="font-mono hidden md:block">
      <ul class="flex space-x-4">
        <li>
          <NuxtLink to="/" class="link">Main</NuxtLink>
        </li>
        <li>
          <NuxtLink to="/about" class="link">About</NuxtLink>
        </li>
        <li>
          <NuxtLink to="/projects" class="link">Projects</NuxtLink>
        </li>
        <li>
          <NuxtLink to="/blog" class="link">Blogs</NuxtLink>
        </li>
        <li>
          <ColorMode />
        </li>
      </ul>
    </nav>
  </div>
</template>

<script setup>
const menuState = ref(false);
const toggleMenu = () => {
  menuState.value = !menuState.value;
};
</script>

<style scoped>
.link {
  @apply p-1 hover:bg-gray-200 dark:hover:bg-gray-700;
}
.link-mobile {
  @apply px-4 py-1  border-b block;
}


.menu-enter-active,
.menu-leave-active {
  transition: all 0.2s;
}

.menu-enter-from,
.menu-leave-to {
  opacity: 0;
}
</style>
